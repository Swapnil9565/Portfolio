...
const Skills = ({ mode }) => {
  const mySkills = [
    ...
    { id: 6, skill: "Tailwind", percentage: 95 },
  ];
  return (
    <div
      className={`${
          ...
          return (
            <div key={id} className='mb-7 md:mb-5'>
              ...
              <div className='w-full bg-[#959595] rounded-md h-[15px]'>
                <div
                  className="bg-red-500 text-center text-white rounded-md h-[15px]" style={{width:`${percentage}%`}}>
                  <span className="h-[30px] w-[30px] rounded-[50%] bg-[#222] float-right mt-[-15px] md:mt-[-10px] mr-[-20px] text-md text-white flex justify-center items-center p-6 md:p-4 md:text-xs">{percentage}%</span>
                </div>
              </div>
            </div>
          );
